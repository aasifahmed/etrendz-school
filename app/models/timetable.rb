class Timetable < ActiveRecord::Base
  ...
  def self.tte_for_the_day(batch,date)
    entries = TimetableEntry.joins(:timetable, :class_timing, :weekday).where("(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.batch_id = ? AND class_timings.is_deleted = ? AND weekdays.is_deleted = ?", date, date, batch.id, false, false).order("class_timings.start_time")
  ...
  def self.tte_for_the_weekday(batch,day)
    date=Date.today
    entries = TimetableEntry.joins(:timetable, :class_timing, :weekday).includes(:employee,:class_timing,:subject).where("(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.batch_id = ? AND class_timings.is_deleted = ? AND weekdays.is_deleted = ?",date, date, batch.id, false, false).order("class_timings.start_time")
    # if entries.empty?
      # today = []
    # else
  ...
  def self.employee_tte(employee,date)
    ...
    entries =[]
    entries += TimetableEntry.joins(:timetable, :class_timing, :weekday).where("(timetables.start_date <= ? AND timetables.end_date >= ?) AND timetable_entries.subject_id in (?) AND timetable_entries.employee_id = (?) AND class_timings.is_deleted = ? AND weekdays.is_deleted = false", date, date, subjects, employee.id, false, false).order("class_timings.start_time")
    entries += TimetableEntry.joins(:timetable, :class_timing, :weekday).where("(timetables.start_date <= ? AND timetables.end_date >= ?) AND timetable_entries.subject_id in (?)  AND class_timings.is_deleted = ? AND weekdays.is_deleted = ?", date, date, elective_subjects, false, false).order("class_timings.start_time")
    # if entries.empty?
      # today=[]
    # else
  ...
  def self.subject_tte(subject_id,date)
    ...
    entries = TimetableEntry.joins(:timetable, :class_timing, :weekday).where("(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.subject_id = ? AND class_timings.is_deleted = ? AND weekdays.is_deleted = ?", date, date, subject.id, false, false)
    # if entries.empty?
      # today=[]
    # else
      today = entries.select{|a| a.day_of_week == date.wday}
