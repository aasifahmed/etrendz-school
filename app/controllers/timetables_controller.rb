...
class TimetablesController < ApplicationController
  ...
  def new_timetable
    ...
    if request.post?
      ...
      p "============time==============="
      p "#{@timetable.start_date }         #{@timetable.end_date }"
      @error = false
      @timetable_errors = []
  ...
  def destroy
    @timetable = Timetable.find(params[:id])
    redirect_to controller: :timetable, notice: t('timetable_deleted') if @timetable.destroy
  end
  ...
  def work_allotment
    ...
    @employees.map{|employee| (@total_time[employee.id] = ((employee.max_hours_week).to_i))}
    if request.post?
  ...
  def timetable_params
    ...
    params.require(:timetable).permit(:start_date, :end_date, :is_active)
  end
